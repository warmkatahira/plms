...
class EmployeeCreateRequest extends BaseRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'statutory_leave_days'                          => 'nullable|numeric|min:0|max:20|regex:/^\d+(\.5)?$/',
            'statutory_leave_remaining_days'                => 'nullable|numeric|min:0|max:20|regex:/^\d+(\.5)?$/',
            'daily_working_hours'                           => ['nullable','numeric','min:0','max:10',Rule::in(array_keys(WorkingHoursEnum::DAILY_WORKING_HOURS)),],
            'half_day_working_hours'                        => ['nullable','numeric','min:0','max:10',Rule::in(array_keys(WorkingHoursEnum::HALF_DAY_WORKING_HOURS)),],
