...
class ItemMallMappingUploadJobs implements ShouldQueue
{
    ...
    public function changeHeaderEn($data_header)
    {
        ...
        foreach($data_header as $header){
            // 英語カラムを定義している配列から取得
            $en_column = ItemMall::column_en_change($header);
            // カラムが空ではない場合
            if($en_column != ''){
                // 配列に変換した英語カラムを格納
                $param[] = $en_column;
    ...
    public function setArrayImportData($chunk, $headers, $chunk_size, $chunk_index)
    {
        ...
        foreach($chunk as $line){
            ...
            foreach($line as $key => $value){
                // 英語カラムを定義している配列から取得
                $en_column = ItemMall::column_en_change($key);
                // カラムが空ではない場合
                if($en_column != ''){
                    // 値の調整を行う
    ...
    public function commonValidation($params, $headers, $chunk_size, $chunk_index)
    {
        ...
        foreach($headers as $column){
            switch($column){
                case 'item_id':
                    $rules += ['*.'.$column => 'required|exists:items,item_id'];
                    break;
                case 'mall_id':
                    $rules += ['*.' . $column => [
                        'required',
                        'exists:malls,mall_id',
                        // item_idとmall_idの重複チェック
                        function($attribute, $value, $fail) use ($params){
                            ...
                            // 行番号が取得でき、かつ対象の行にitem_idが存在する場合に処理を実行
                            if(!is_null($row_index) && isset($params[$row_index]['item_id'])){
                                // 現在の行のitem_idとmall_idを取得
                                $current_item_id = $params[$row_index]['item_id'];
                                $current_mall_id = $value;
                                // item_idとmall_idを結合して変数に格納
                                $combination = "{$current_item_id}-{$current_mall_id}";
                                // 確認された組み合わせを格納する配列を初期化(同じ呼び出し内で最初の1回だけ初期化)
                                static $seen = [];
                                // 配列に今回の組み合わせが存在していた場合
                                if(in_array($combination, $seen, true)){
                                    // エラーを返す
                                    $fail("item_id：{$current_item_id} と mall_id：{$current_mall_id} の組み合わせが重複しています。");
        ...
        $attributes = [
            '*.item_id'             => '商品ID',
            '*.mall_id'             => 'モールID',
            '*.mall_item_code'      => 'モール商品コード',
            '*.mall_variation_code' => 'モールバリエーションコード',
        ];
        // バリデーション実施
    ...
    {
        // 一時テーブルを作成
        DB::statement('DROP TEMPORARY TABLE IF EXISTS temp_item_mall');
        DB::statement('
            CREATE TEMPORARY TABLE temp_item_mall (
                item_id INT UNSIGNED NOT NULL,
                mall_id INT UNSIGNED NOT NULL,
                mall_item_code VARCHAR(255) NOT NULL,
                mall_variation_code VARCHAR(255) NULL
            )
        ');
        // 一時テーブルに追加
        DB::table('temp_item_mall')->insert($create_data);
    }

    public function procCreateAndUpdate($headers, $upload_type, $nowDate)
    {
        // +-+-+-+-+-+-+-+-+-   商品IDとモールIDの組み合わせが存在しない場合は、追加処理を行う   +-+-+-+-+-+-+-+-+-
        // temp_item_mallテーブルにしか存在していないレコードを取得
        if($upload_type === ItemUploadEnum::UPLOAD_TYPE_CREATE){
            return DB::affectingStatement("
                INSERT INTO item_mall (item_id, mall_id, mall_item_code, mall_variation_code, created_at, updated_at)
                SELECT 
                    t.item_id,
                    t.mall_id,
                    t.mall_item_code,
                    t.mall_variation_code,
                    NOW(),
                    NOW()
                FROM temp_item_mall t
                LEFT JOIN item_mall im
                    ON im.item_id = t.item_id AND im.mall_id = t.mall_id
                WHERE im.item_id IS NULL
            ");
        }
        // +-+-+-+-+-+-+-+-+-   商品IDとモールIDの組み合わせがitem_mallテーブルに存在する場合は、更新処理を行う   +-+-+-+-+-+-+-+-+-
        // item_mallテーブルとtemp_item_mallテーブルを結合して更新に必要なカラムを取得（結合した結果、どっちのテーブルにも存在しているデータ）
        if($upload_type === ItemUploadEnum::UPLOAD_TYPE_UPDATE){
            return DB::table('item_mall')
                ->join('temp_item_mall', function($join) {
                    $join->on('item_mall.item_id', '=', 'temp_item_mall.item_id')
                        ->on('item_mall.mall_id', '=', 'temp_item_mall.mall_id');
                })
                ->update([
                    'item_mall.mall_item_code'      => DB::raw('temp_item_mall.mall_item_code'),
                    'item_mall.mall_variation_code' => DB::raw('temp_item_mall.mall_variation_code'),
                    'item_mall.updated_at'          => $nowDate
                ]);
        }
    }
}