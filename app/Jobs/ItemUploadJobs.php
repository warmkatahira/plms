...
class ItemUploadJobs implements ShouldQueue
{
    ...
    public function valueAdjustment($key, $value, $shipping_method_arr)
    {
        ...
        switch($key){
            ...
            case '出荷検品要否':
                // 否を「0」、要を「1」に変換
                $adjustment_value = $value === '否' ? 0 : ($value === '要' ? 1 : $value);
                break;
            case '納品書表示':
                // 表示を「0」、非表示を「1」に変換
                $adjustment_value = $value === '表示' ? 0 : ($value === '非表示' ? 1 : $value);
                break;
            default:
                // 何もしない
    ...
    public function commonValidation($params, $headers, $chunk_size, $chunk_index, $upload_type)
    {
        ...
        foreach($headers as $column){
            switch ($column){
                ...
                case 'item_jan_code':
                    $rules += ['*.' . $column => [
                            'required_if:*.is_shipping_inspection_required,1',
                            'max:13',
                    ]];
                    break;
                case 'item_name':
                ...
                case 'item_category_2':
                    $rules += ['*.'.$column => 'nullable|max:100'];
                    break;
                case 'is_stock_managed':
                case 'is_shipping_inspection_required':
                case 'is_hide_on_delivery_note':
    ...
    {
        // テーブルをロック
        ItemImport::select()->lockForUpdate()->get();
        // 追加先のテーブルをクリア
