...
class InfoGetService
{
    ...
    public function getInfo($from, $to)
    {
        ...
        // 作業前の注文の出荷数量を取得
        $orders = Order::getOrderSpecifyOrderStatus('<=', OrderStatusEnum::SHUKKA_MACHI)->with('order_items.order_item_components')->get();
        $sagyo_mae_ship_quantity = $orders->flatMap(function($order) {
                                        return $order->order_items->flatMap(function($item) {
                                            return $item->order_item_components;
                                        });
                                    })->sum('ship_quantity');
        // 作業中の注文件数を取得
        $sagyo_chu_order_count = Order::getOrderSpecifyOrderStatus('=', OrderStatusEnum::SAGYO_CHU)->get()->count();
        // 作業中の注文の出荷数量を取得
        $orders = Order::getOrderSpecifyOrderStatus('=', OrderStatusEnum::SAGYO_CHU)->with('order_items.order_item_components')->get();
