...
class ItemUpdateService
{
    ...
    public function checkUpdatableItem($request)
    {
        ...
        if($item->is_stock_managed != $request->is_stock_managed){
            ...
            $exists = OrderItemComponent::where('allocated_component_item_id', $item->item_id)
                        ->whereHas('order_item.order', function ($query) {
                            $query->where('order_status_id', '!=', OrderStatusEnum::SHUKKA_ZUMI); // 出荷完了前の受注
                        })
                        ->exists();
            // レコードが取得できている場合
            if($exists){
    ...
    public function updateItem($request)
    {
        ...
        $item->update([
            ...
            'is_shipping_inspection_required'   => $request->is_shipping_inspection_required,
            'is_hide_on_delivery_note'          => $request->is_hide_on_delivery_note,
