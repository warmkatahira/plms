<?php

namespace App\Services\Order\OrderDelete;

// モデル
use App\Models\Order;
use App\Models\OrderItem;
use App\Models\OrderItemComponent;
use App\Models\Stock;
use App\Models\Mall;
// サービス
use App\Services\API\Makeshop\MakeshopOrderCancelService;
use App\Services\API\ApiCommonService;
// 列挙
use App\Enums\OrderStatusEnum;
use App\Enums\MallEnum;
use App\Enums\API\ApiMasterEnum;
// その他
use Illuminate\Support\Facades\DB;

class OrderDeleteService
{
    // 削除できる受注であるか確認
    public function checkDeletable($chk)
    {
        // 対象をロック
        $orders = Order::whereIn('order_control_id', $chk)->lockForUpdate()->get();
        $order_items = OrderItem::whereIn('order_control_id', $chk)->lockForUpdate()->get();
        OrderItemComponent::whereIn('order_item_id', $order_items->pluck('order_item_id'))->lockForUpdate()->get();
        // 注文ステータスが「出荷待ち」より大きい対象が存在する場合
        if($orders->where('order_status_id', '>', OrderStatusEnum::SHUKKA_MACHI)->count() > 0){
            throw new \RuntimeException('削除できない注文ステータスが含まれています。');
        }
    }

    // 引当済みの在庫数を戻す(在庫管理を行っている商品だけを取得)
    public function incrementAllocatedStockBackToAvailableStock($chk)
    {
        // 削除対象で在庫が引当済みになっている商品を取得
        $allocated_items = Order::whereIn('orders.order_control_id', $chk)
                            ->where('order_item_components.is_stock_managed_snapshot', true)
                            ->join('order_items', 'order_items.order_control_id', 'orders.order_control_id')
                            ->join('order_item_components', 'order_item_components.order_item_id', 'order_items.order_item_id')
                            ->select(DB::raw("sum(order_item_components.ship_quantity - order_item_components.unallocated_quantity) as total_allocated_quantity, shipping_base_id, order_item_components.allocated_component_item_id as item_id"))
                            ->groupBy('shipping_base_id', 'order_item_components.allocated_component_item_id')
                            ->having('total_allocated_quantity', '>', 0)
                            ->get();
        // 引当済みの商品の分だけループ処理
        $stocks = Stock::where(function ($query) use ($allocated_items) {
            foreach($allocated_items as $index => $allocated_item){
                // インデックスによってメソッドを可変
                $queryMethod = $index === 0 ? 'where' : 'orWhere';
                // 条件を適用
                $query->$queryMethod(function ($subQuery) use ($allocated_item) {
                    $subQuery->where('base_id', $allocated_item->shipping_base_id)
                            ->where('item_id', $allocated_item->item_id);
                });
            }
        });
        // 在庫をロック
        $stocks = $stocks->lockForUpdate()->get();
        // 引当済みの商品の分だけループ処理
        foreach($allocated_items as $allocated_item){
            // 引当済みの在庫数を有効在庫数に加算（戻す）
            Stock::where('base_id', $allocated_item->shipping_base_id)
                    ->where('item_id', $allocated_item->item_id)
                    ->increment('available_stock', ($allocated_item->total_allocated_quantity));
        }
    }

    // モール側の注文キャンセル処理
    public function cancelOrderForMall($chk)
    {
        // インスタンス化
        $ApiCommonService = new ApiCommonService;
        // モールを取得
        $malls = Mall::getAll()->get();
        // モールの分だけループ処理
        foreach($malls as $mall){
            // 削除対象からモールを条件に注文を取得
            $orders = Order::whereIn('order_control_id', $chk)
                        ->join('order_categories', 'order_categories.order_category_id', 'orders.order_category_id')
                        ->where('mall_id', $mall->mall_id)
                        ->get();
            // 注文がある場合
            if($orders->isNotEmpty()){
                // makeshopの場合
                if($mall->mall_id === MallEnum::MAKESHOP){
                    // インスタンス化
                    $MakeshopOrderCancelService = new MakeshopOrderCancelService;
                    // 注文をキャンセル
                    $errors = $MakeshopOrderCancelService->cancelOrder($orders);
                    // エラーをファイルに出力
                    $file_name = $MakeshopOrderCancelService->exportError($errors);
                    // API履歴を追加
                    $ApiCommonService->createApiHistory($file_name, MallEnum::MAKESHOP, ApiMasterEnum::API_ACTION_ORDER_CANCEL);
                }
            }
        }
    }

    // 受注を削除
    public function deleteOrder($chk)
    {
        // 受注を削除
        Order::whereIn('order_control_id', $chk)->delete();
    }
}