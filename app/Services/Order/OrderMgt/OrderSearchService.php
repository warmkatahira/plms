...
class OrderSearchService
{
    ...
    public function deleteSession()
    {
        session()->forget([
            ...
            'search_order_mark',
            'search_is_shipping_method_changed',
        ]);
        // 出荷管理以外のページの場合
    ...
    public function setSearchCondition($request)
    {
        ...
        if($request->search_type === 'search'){
            ...
            session(['search_is_shipping_method_changed' => $request->search_is_shipping_method_changed]);
        }
    }
    ...
    public function getSearchResult()
    {
        ...
        $query = Order::where('order_status_id', session('search_order_status_id'))
                    ->with('order_category.mall')
                    ->with('shipping_method.delivery_company')
                    ->with('order_items.order_item_components.item')
                    ->with('order_items.item')
                    ->with('order_items.set_item');
        // 出荷日の条件がある場合
        ...
        // 配送方法変更の条件がある場合
        if(session('search_is_shipping_method_changed') != null){
            // 条件を指定して取得
            $query = $query->where('is_shipping_method_changed', session('search_is_shipping_method_changed'));
        }
        // 並び替えを実施
