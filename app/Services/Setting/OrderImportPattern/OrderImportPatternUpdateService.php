...
class OrderImportPatternUpdateService
{
    // 受注取込パターンを更新
    public function updateOrderImportPattern($request)
    {
        // 受注取込パターンを取得
        $order_import_pattern = OrderImportPattern::getSpecify($request->order_import_pattern_id)->first();
        // 受注取込パターンを更新
        $order_import_pattern->update([
        ...
        // 現状の受注取込パターン詳細を削除
        $order_import_pattern->order_import_pattern_details()->delete();
