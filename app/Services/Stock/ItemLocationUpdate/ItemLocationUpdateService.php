...
class ItemLocationUpdateService
{
    ...
    public function checkHeader($save_file_path)
    {
        ...
        foreach($require_header as $header){
            ...
            // nullではない場合
            if(!is_null($result)){
                throw new \RuntimeException('ファイルが正しくないため、取り込みできませんでした。');
            }
        }
    }
    ...
    public function setArrayImport($save_file_path)
    {
        ...
        if(count(array_filter($validation_error)) != 0){
            throw new \RuntimeException('データが正しくないため、取り込みできませんでした。');
        }
        return $update_data;
    }
    ...
    // レコードが存在しない商品ロケーション更新対象を追加
    public function createNoStockRecord($update_data)
    {
        // 在庫操作する分だけループ処理
        foreach($update_data as $data){
            // 倉庫IDと商品IDを指定して在庫を取得
            $stock = Stock::getSpecifyByBaseIdItemId($data['base_id'], $data['item_id'])->first();
            // レコードが取得できていない場合
            if(is_null($stock)){
                // レコードを追加
                Stock::create([
