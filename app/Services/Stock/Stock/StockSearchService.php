...
class StockSearchService
{
    ...
    public function getSearchResult($query, $route_name)
    {
        ...
        // 出荷数を商品×出荷倉庫毎で取得
        $ship_quantity_sub_query = Order::join('order_items', 'order_items.order_control_id', 'orders.order_control_id')
                                        ->join('order_item_components', 'order_item_components.order_item_id', 'order_items.order_item_id')
                                        ->join('items', 'items.item_id', 'order_item_components.allocated_component_item_id')
        ...
        // queryとship_quantity_sub_queryを結合
        $query = $query->leftJoinSub($ship_quantity_sub_query, 'ship_quantity_sub_query', function($join){
            $join->on('ship_quantity_sub_query.item_id', '=', 'item_base.item_id')
                ->on('ship_quantity_sub_query.shipping_base_id', '=', 'item_base.base_id');
        });
        // 商品単位表示の場合
        if($route_name === RouteNameEnum::STOCK_BY_ITEM){
            ...
            foreach ($bases as $base){
                $query->addSelect(DB::raw("
                    ...
                    SUM(CASE WHEN item_base.base_id = '{$base->base_id}' THEN ship_quantity_sub_query.total_ship_quantity ELSE 0 END) as total_ship_quantity_{$base->base_id}
                "));
            }
            // グループ化
            $query = $query->groupBy(
        ...
        if($route_name === RouteNameEnum::STOCK_BY_STOCK || $route_name === RouteNameEnum::INPUT_STOCK_OPERATION){
            ...
            $query->addSelect(
                ...
                DB::raw('IFNULL(ship_quantity_sub_query.total_ship_quantity, 0) as total_ship_quantity'),
