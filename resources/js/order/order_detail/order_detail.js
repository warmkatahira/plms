...
$(document).on('click', function(e){
    ...
    // クリックされた要素にモーダルを閉じるクラス名が設定されていれば、モーダルを閉じる
    if(e.target.classList.contains('receipt_name_update_modal_close') === true){
        $('#receipt_name_update_modal').addClass('hidden');
    }
    // クリックした要素のIDがモーダルを開くものであれば、モーダルを開く
    if(e.target.id === 'receipt_name_update_modal_open'){
        // テキストボックスを現在の値に変更
        $('#receipt_name').val($('#current_receipt_name').val());
        $('#receipt_name_update_modal').removeClass('hidden');
    }
    // クリックされた要素にモーダルを閉じるクラス名が設定されていれば、モーダルを閉じる
...
// 領収書宛名の更新ボタンを押下した場合
$('#receipt_name_update_enter').on("click",function(){
    // 処理を実行するか確認
    const result = window.confirm("領収書宛名の更新を実行しますか？");
    // 「はい」が押下されたらsubmit、「いいえ」が押下されたら処理キャンセル
    if(result === true){
        start_loading();
        $("#receipt_name_update_form").submit();
    }
});
...
// 領収書宛名更新モーダルのツールチップ
tippy('.tippy_receipt_name_update', {
    content: "領収書宛名を更新する場合は、<br>こちらをクリックして下さい",
    duration: 500,
    maxWidth: 'none',
    allowHTML: true,
    placement: 'right',
