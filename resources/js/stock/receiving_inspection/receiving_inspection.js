        ...
        $.ajax({
            ...
            success: function(data){
                try {
                    ...
                    if(data['add']){
                        // 新しい行を作成
                        const newRow = $('<tr>').attr('id', data.item['item_id']).addClass('text-xs');
                        // buttonタグを作成
                        const updateButton = $('<button>')
                                                .attr('type', 'button')
                                                .addClass('btn item_change_modal_open bg-btn-enter text-white py-1 px-2')
                                                .attr('data-item-jan-code', data.item['item_jan_code'])
                                                .attr('data-item-id', data.item['item_id'])
                                                .text('変更');
                        // buttonタグを作成
                        ...
                        const newCell2 = $('<td>').text(data.item['item_code']).addClass('py-1 px-2 border');
                        const newCell3 = $('<td>').text(data.item['item_jan_code']).addClass('py-1 px-2 border');
                        const newCell4 = $('<td>').text(data.item['item_name']).addClass('py-1 px-2 border');
                        const newCell5 = $('<td>').text(data['quantity']).addClass('inspection_quantity py-1 px-2 border text-right').attr('id', data.item['item_id'] + '_quantity');
                        // セルを行に追加
                    ...
                    if(!data['add']){
                        $('#' + data.item['item_id'] + '_quantity').text(data['quantity']);
                    }
                    // 検品数量合計を更新
