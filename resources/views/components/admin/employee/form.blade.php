<form method="POST"
    ...
    <div class="flex flex-col border border-gray-400 divide-y divide-gray-400">
        ...
        <x-form.select label="1日あたりの時間数" id="daily_working_hours" name="daily_working_hours" :items="$daily_working_hours" optionValue="working_hour" optionText="working_hour" :value="$form_mode === 'update' ? $employee->paid_leave->daily_working_hours : null" />
        <x-form.select label="半日あたりの時間数" id="half_day_working_hours" name="half_day_working_hours" :items="$half_day_working_hours" optionValue="working_hour" optionText="working_hour" :value="$form_mode === 'update' ? $employee->paid_leave->half_day_working_hours : null" />
        ...
        <x-form.input type="date" label="義務の期限" id="statutory_leave_expiration_date" name="statutory_leave_expiration_date" :value="$form_mode === 'update' ? $employee->statutory_leave->statutory_leave_expiration_date : null" />
        <x-form.input type="tel" label="義務の日数" id="statutory_leave_days" name="statutory_leave_days" :value="$form_mode === 'update' ? $employee->statutory_leave->statutory_leave_days : null" />
        <x-form.input type="tel" label="義務の残日数" id="statutory_leave_remaining_days" name="statutory_leave_remaining_days" :value="$form_mode === 'update' ? $employee->statutory_leave->statutory_leave_remaining_days : null" />
    </div>
    @if($form_mode === 'update')
