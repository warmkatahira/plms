<form method="POST"
    ...
    <div class="flex flex-col border border-gray-400 divide-y divide-gray-400">
        ...
        <x-form.select-array label="1日あたりの時間数" id="daily_working_hours" name="daily_working_hours" :items="$daily_working_hours" :value="$form_mode === 'update' ? $employee->paid_leave->daily_working_hours : null" required="true" />
        <x-form.select-array label="半日あたりの時間数" id="half_day_working_hours" name="half_day_working_hours" :items="$half_day_working_hours" :value="$form_mode === 'update' ? $employee->paid_leave->half_day_working_hours : null" required="true" />
        <x-form.switch-boolean label="義務残日数自動更新" id="is_auto_update_statutory_leave_remaining_days" name="is_auto_update_statutory_leave_remaining_days"  label1="有効" label0="無効" :value="$form_mode === 'update' ? $employee->is_auto_update_statutory_leave_remaining_days : null" required="true" />
        <x-form.input type="date" label="義務期限日" id="statutory_leave_expiration_date" name="statutory_leave_expiration_date" :value="$form_mode === 'update' ? $employee->statutory_leave->statutory_leave_expiration_date : null" required="true" />
        <x-form.input type="tel" label="義務日数" id="statutory_leave_days" name="statutory_leave_days" :value="$form_mode === 'update' ? $employee->statutory_leave->statutory_leave_days : null" required="true" />
        <x-form.input type="tel" label="義務残日数" id="statutory_leave_remaining_days" name="statutory_leave_remaining_days" :value="$form_mode === 'update' ? $employee->statutory_leave->statutory_leave_remaining_days : null" required="true" />
    </div>
    @if($form_mode === 'update')
