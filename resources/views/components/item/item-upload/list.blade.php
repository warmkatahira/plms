            ...
            <tbody class="bg-white">
                @foreach($itemUploadHistories as $item_upload_history)
                    <tr class="text-left cursor-default whitespace-nowrap">
                        ...
                        <td class="py-1 px-2 border">{{ $item_upload_history->upload_target_text }}</td>
                        ...
                        <td class="py-1 px-2 border">
                            @if(!is_null($item_upload_history->error_file_name))
                                <a href="{{ route('error_file_download.download', ['filename' => $item_upload_history->error_file_name]) }}" class="text-center text-blue-500"><i class="las la-cloud-download-alt mr-1 la-lg"></i>ダウンロード</a>
                            @endif
                        </td>
