<form method="GET" action="{{ route($route) }}" id="search_form">
    ...
    <div class="flex flex-col gap-y-2 p-3 bg-white min-w-60 text-xs border border-black">
        <x-search.date-period type="date" fromId="search_import_date_from" fromName="search_import_date_from" toId="search_import_date_to" toName="search_import_date_to" label="取込日" />
