<form method="GET" action="{{ route($route) }}" id="search_form">
    ...
    <div class="flex flex-col gap-y-2 p-3 bg-white min-w-60 text-xs border border-black">
        <x-search.input type="text" label="注文番号" id="search_order_no" name="search_order_no" />
        <x-search.input type="text" label="受注管理ID" id="search_order_control_id" name="search_order_control_id" />
        <x-search.datalist label="受注マーク" id="search_order_mark" name="search_order_mark" listId="order_mark" :selectItems="$orderMarks" optionValue="order_mark" />
        ...
        <x-search.select label="出荷倉庫" id="search_shipping_base_id" name="search_shipping_base_id" :selectItems="$bases" optionValue="base_id" optionText="base_name" />
        <x-search.input type="text" label="配送先名" id="search_ship_name" name="search_ship_name" />
        <x-search.select-delivery-company :deliveryCompanies="$deliveryCompanies" />
        <x-search.select-boolean label="配送方法変更" id="search_is_shipping_method_changed" name="search_is_shipping_method_changed" label0="未変更" label1="変更済" />
        <x-search.select-boolean label="出荷検品状態" id="search_is_shipping_inspection_complete" name="search_is_shipping_inspection_complete" label0="未完了" label1="完了" />
