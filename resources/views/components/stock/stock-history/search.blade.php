<form method="GET" action="{{ route($route) }}" id="search_form">
    ...
    <div class="flex flex-col gap-y-2 p-3 bg-white min-w-60 text-xs border border-black">
        <x-search.date-period type="date" label="日付" fromId="search_stock_history_date_from" fromName="search_stock_history_date_from" toId="search_stock_history_date_to" toName="search_stock_history_date_to" />
        <x-search.select label="区分" id="search_stock_history_category_id" name="search_stock_history_category_id" :selectItems="$stockHistoryCategories" optionValue="stock_history_category_id" optionText="stock_history_category_name" />
