    ...
    <div class="page-container">
        <div class="flex flex-row">
            <div class="flex flex-col">
                ...
                <span>{{ SystemEnum::getSystemTitle() }}</span>
            </div>
            <div class="flex text-center text-2xl ml-auto mr-1">
                <p class="bg-theme-sub border border-black py-2 px-5">合計数</p>
                <p class="py-2 px-10 border-y border-r border-black">{{ number_format($report_total_ship_quantity) }}</p>
            </div>
        </div>
        <table class="w-full mt-5">
            <thead>
                <tr class="bg-theme-sub">
                    ...
                    <th class="ship_quantity text-right py-2 font-thin pr-2">数量</th>
            ...
            <tbody>
                ...
                @foreach($items as $item)
                    ...
                    <tr class="@if($count % 2 == 0) bg-gray-200 @endif border-y border-black">
                        ...
                        <td class="ship_quantity text-right py-2 pr-2 text-2xl">{{ number_format($item->total_ship_quantity) }}</td>
