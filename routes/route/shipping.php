...
// +-+-+-+-+-+-+-+- ABC分析 +-+-+-+-+-+-+-+-
use App\Http\Controllers\Shipping\AbcAnalysis\AbcAnalysisController;
use App\Http\Controllers\Shipping\AbcAnalysis\AbcAnalysisDownloadController;
...
Route::middleware('common')->group(function (){
    ...
    Route::middleware(['warm_check'])->group(function () {
        ...
        Route::controller(KobetsuPickingListCreateController::class)->prefix('kobetsu_picking_list_create')->name('kobetsu_picking_list_create.')->group(function(){
            Route::get('create', 'create')->name('create');
            Route::post('create_by_select_order', 'create_by_select_order')->name('create_by_select_order');
        });
        // -+-+-+-+-+-+-+-+-+-+-+-+ 納品書 -+-+-+-+-+-+-+-+-+-+-+-+
        ...
        Route::controller(NifudaCreateController::class)->prefix('nifuda_create')->name('nifuda_create.')->group(function(){
            Route::get('create_by_shipping_method', 'create_by_shipping_method')->name('create_by_shipping_method');
            Route::post('create_by_select_order', 'create_by_select_order')->name('create_by_select_order');
        });
        ...
        // +-+-+-+-+-+-+-+- ABC分析 +-+-+-+-+-+-+-+-
        Route::controller(AbcAnalysisController::class)->prefix('abc_analysis')->name('abc_analysis.')->group(function(){
            Route::get('', 'index')->name('index');
            Route::get('ajax_get_chart_data', 'ajax_get_chart_data');
        });
        Route::controller(AbcAnalysisDownloadController::class)->prefix('abc_analysis_download')->name('abc_analysis_download.')->group(function(){
            Route::get('download', 'download')->name('download');
        });
    });
});